...
if upload is not None:
    ...
    if run2 == True:
        ...
        dfx.rename(columns = {dfx.columns.values[1]:Coluna+'_x'}, inplace = True)
        dfx.rename(columns = {dfx.columns.values[3]:Coluna+'_y'}, inplace = True)
        c = dfx.groupby(['Descrição',Coluna+'_y'])[Coluna+'_x'].max()
